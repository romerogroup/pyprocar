...
def f_evec(args):
    ...
    evec = DFTB_evec(
        filename=args.eigenvectors, verbose=args.verbose
    ...
    # otherwise, a plain set of kpoints is spanned. Physically meaningless
        ...
        print("A list of k-points was not provided. Creating a fake and meaningless of K-points")
...
if __name__ == "__main__":
    ...
    p_evec = subparsers.add_parser(
        "evec", help="Converts the DFTB+ Eigenvectors (text-file) into a vasp PROCAR file."
    )
    ...
    p_evec.add_argument(
        "--fermi", default=0.0, type=float, help="Supersedes the extracted from `detailed.out`"
    )
    p_evec.add_argument(
        "--detailed",
        "-d",
        default="detailed.out",
        help="The file with the Fermi energy (detailed.out)",
    )
    p_evec.add_argument(
        "-k",
        "--kpointsfile",
        help="file with the explicit list of K-points used (detailed.xml)`",
        default="detailed.xml",
    )
    ...
    p_input = subparsers.add_parser(
        "input", help="add the tags needed for pyprocar to a dftb+ input"
    )
    ...
    p_input.add_argument(
        "-o", "--output", default="dftb_in.hsd", help="output file. Default: dftb_in.hsd"
    )
