...
def call_bandscompare(args):
	"""
	This module calls the bandscompare function.
	"""
	pyprocar.bandscompare(
		...
		mode = args.mode,
		abinit_output = args.abinit,
		abinit_output2 = args.abinit2,
		spin = args.spin,
		spin2 =	args.spin2,
		atoms = args.atoms,
		atoms2 = args.atoms2,
		orbitals = args.orbitals,
		orbitals2 = args.orbitals2,
		fermi = args.fermi,
		fermi2 = args.fermi2, 
		elimit = args.elimit,
		mask = args.mask,
		markersize = args.markersize,
		markersize2 = args.markdersize2,
		cmap = args.cmap,
		vmax = args.vmax,
		vmin = args.vmin,
		vmax2 = args.vmax2,
...
################### bandscompare ######################################################
	parserBandscompare = subparsers.add_parser('bandscompare',help ='Compare band structure',formatter_class=RawTextHelpFormatter)

	phelp = ("Input file. It can be compressed")
	parserBandscompare.add_argument('file' , help=phelp)
	phelp = ("Second input file. It can be compressed")
	parserBandscompare.add_argument('file2' , help=phelp)
	...
	"  but useful to visualize energy level. Only 1 K-point!\n\n")
	choices = ["scatter","plain","parametric", "atomic"]
	parserBandscompare.add_argument("-m", "--mode", help=phelp, default="plain",choices=choices)

	phelp = ("Color of the bands for plain mode for first plot.")
	parserBandscompare.add_argument('-color' , help=phelp,default='blue')
	phelp = ("Color of the bands for plain mode for second plot.")
	parserBandscompare.add_argument('-color2' , help=phelp,default='blue')

	phelp = ("Name of Abinit output file if used.")
	parserBandscompare.add_argument('-abinit' , help=phelp,default=None)
	phelp = ("Name of second Abinit output file if used.")
	parserBandscompare.add_argument('-abinit2' , help=phelp,default=None)
	...
	"(kx,ky) vector. Useful for Rashba-like states in surfaces. Use\n "
	"'--cmap seismic'\n\n ")
	parserBandscompare.add_argument("-s", "--spin" , choices=['0','1','2','3', 'st'],default='0', help=phelp)
	parserBandscompare.add_argument("-s2", "--spin2" , choices=['0','1','2','3', 'st'],default='0', help='Spin for second plot.')
	...
	"-a 0 2 :  select the 1st  and 3rd. rows (likely 1st and 3rd atoms)"
	"\n\n")
	parserBandscompare.add_argument("-a", "--atoms" , type=int, nargs='+',help=phelp,default=None)
	parserBandscompare.add_argument("-a2", "--atoms2" , type=int, nargs='+',help='Atoms for second plot.',default=None)
	...
	"-o 4 5 6 7 8 : all the d-orbitasl (unless...)\n"
	"-o 2 6 : pz+dzz (did you look at the PROCAR?)\n\n ")
	parserBandscompare.add_argument("-o", "--orbitals" , type=int, nargs='+',help=phelp,default=None)
	parserBandscompare.add_argument("-o2", "--orbitals2" , type=int, nargs='+',help='Orbitals for second plot.',default=None)
	...
	"Mind: The Fermi energy MUST be the one from the self-consistent\n"
	"calculation, not from a Bandstructure calculation!\n\n")
	parserBandscompare.add_argument("-f", "--fermi" , type=float, help=phelp,nargs='+',default=None)
	parserBandscompare.add_argument("-f2", "--fermi2" , type=float, help='Fermi energy for second plot.',nargs='+',default=None)

	phelp = ("Min/Max energy to be ploted. Example:\n "
	"--elimit -1 1 : From -1 to 1 around Fermi energy (if given)\n\n")
	parserBandscompare.add_argument("--elimit" , type=float, nargs=2, help=phelp,default=None)
	...
	"other small value). Mind: it works with the absolute value of\n"
	"projection (no problem with spin polarization)\n\n")
	parserBandscompare.add_argument("--mask" , type=float, help=phelp,default=None)

	phelp = ("Size of markers, if used. Each mode has it own scale,\n"
	"just test them\n\n")
	parserBandscompare.add_argument("--markersize", type=float, help=phelp,default=0.02)
	parserBandscompare.add_argument("--markersize2", type=float, help='Markersize for second plot.',default=0.02)
	...
	"  depending of spin channel\n"
	"--cmap  seismic_r : the 'seismic' colormap, but reversed.\n\n")
	parserBandscompare.add_argument("--cmap" , help=phelp, default="jet")
	...
	"--vmax 0.2: If you want to capture some tiny effect, eg: s-band of\n"
	"  a impurity on a metal\n\n")
	parserBandscompare.add_argument("--vmax" , type=float, help=phelp,default=None)
	parserBandscompare.add_argument("--vmax2" , type=float, help='vmax for second plot.',default=None)

	phelp = ("Like '--vmax' (see '--vmax'), However, for spin-polarized \n"
	"(collinear or not) you can set it to a negative value. Actually\n"
	"you can do it for a non-spin-polarized calculation and the \n"
	"effect will be a 'stretching' of the color scheme, try it.\n\n")
	parserBandscompare.add_argument("--vmin" , type=float, help=phelp,default=None)  
	parserBandscompare.add_argument("--vmin2" , type=float, help='vmin for second plot.',default=None)

	phelp = ("switch on/off the grid. Default is 'on'\n\n")
	parserBandscompare.add_argument("--grid", type=bool, help=phelp, default=True)

	phelp = ("set the marker shape, ie: 'o'=circle, 's'=square,\ '-'=line\n"
	"(only mode `plain`, other symbols: google pyplot markers)\n\n")
	parserBandscompare.add_argument("--marker", type=str, help=phelp, default='o')
	parserBandscompare.add_argument("--marker2", type=str, help='Marker for second plot.', default='o')

	phelp = ("Some fault tolerance for ill-formatted files (stupid fortran)\n"
	"But be careful, something could be messed up and don't work (at\n"
	"least as expected). Length of K-points paths will be ignored\n\n")
	parserBandscompare.add_argument("--permissive", help=phelp, action='store_true')

	phelp = ("Enable human-like 1-based order (ie 1st is 1, 2nd is 2, and so\n"
	"on). Mind: this only works for atoms, not for orbitals or spin\n\n")
	parserBandscompare.add_argument("-u", "--human", help=phelp,action="store_true")

	phelp = ("Saves the figure, instead of display it on screen. Anyway, you can\n"
	"save from the screen too. Any file extension supported by\n "
	"`matplotlib.savefig` is valid (if you are too lazy to google it,\n"
	"trial and error also works fine)\n\n")
	parserBandscompare.add_argument('--savefig' , help=phelp,default=None)

	phelp = ("list of ticks along the kpoints axis (x axis). For instance a\n"
	"bandstructure G-X-M with 10 point by segment should be:\n "
	"--kticks 0 9 19\n\n")
	parserBandscompare.add_argument('--kticks' , help=phelp, nargs='+', type=int)

	phelp = ("Names of the points given in `--kticks`. In the `kticks` example\n"
	"they should be `--knames \"\$Gamma\$\" X M`. As you can see \n"
	"LaTeX stuff works with a minimal mess (extra \\s)\n\n")
	parserBandscompare.add_argument('--knames' , help=phelp, nargs='+', type=str,default=None)

	phelp = ("Title, to use several words, use quotation marks\"\" or ''. Latex\n"
	" works if you scape the special characteres, ie: $\\alpha$ -> \n"
	"\$\\\\alpha\$")
	parserBandscompare.add_argument('-t', '--title' , help=phelp, type=str,default=None)
	...
	"(however, the basis and reciprocal vectors will be safe from the\n"
	"non-self-consistentness)\n\n")
	parserBandscompare.add_argument('--outcar', help=phelp,default=None)
	parserBandscompare.add_argument('--outcar2', help='OUTCAR for second plot.',default=None)

	phelp = ("KPOINTS file for bandstructure plotting.\n")
	parserBandscompare.add_argument('--kpointsfile', help=phelp,default=None)

	parserBandscompare.add_argument('-legend', help='Legend name for first plot.',default='PROCAR1')
	parserBandscompare.add_argument('-legend2', help='Legend name for second plot',default='PROCAR2')

	parserBandscompare.set_defaults(func=call_bandscompare)
