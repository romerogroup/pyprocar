...
def cat(
    ...
    if mergeparallel is False and fixformat is False:
        if gz is True:
            print("out compressed: true")

        if gz is True and outFile[-3:] != ".gz":
            outFile += ".gz"
    ...
    if mergeparallel is True and fixformat is False:
        _mergeparallel(inFiles, outFile, nspin, abinit_output)

    elif mergeparallel is True and fixformat is True:
    ...
    elif mergeparallel is False and fixformat is True:
        print("Using fixformat = True without mergeparallel. Input a single PROCAR.")
        _fixformat(inFiles[0] if inFiles else "", outFile)
...
def _mergeparallel(
    ...
    if abinit_output:
        abinitparserobject = AbinitOutput(filepath=abinit_output)
...
def _fixformat(inputfile: str, outputfile: str) -> None:
    ...
    # header
    header = re.findall(r"#\sof\s.*", data)[0]
    ...
    kpoints_raw = re.findall(r"k-point\s*[0-9]\s*:*.*", data)

    for kpoint_counter in range(len(kpoints_raw)):
        if kpoint_counter == (len(kpoints_raw) - 1):
            # get bands of last k point
            bands_raw = re.findall(kpoints_raw[kpoint_counter] + r"([a-z0-9\s\n.+#-]*)", data)[0]
        ...
        raw_bands = re.findall(r"band\s*[0-9]*.*", bands_raw)
        ...
        for band_counter in range(len(raw_bands)):
            if band_counter == (len(raw_bands) - 1):
                # the last band
                single_band = re.findall(raw_bands[band_counter] + r"([a-z0-9.+\s\n-]*)", bands_raw)[
                    0
                ]
                ...
                single_band = re.findall(
                    raw_bands[band_counter] + r"([a-z0-9.+\s\n-]*)" + raw_bands[band_counter + 1],
                    bands_raw,
                )[0]

            # get the column headers for ion, orbitals and total
            column_header = re.findall(r"ion\s.*tot", single_band)[0]

            # get number of ions using PROCAR file
            nion_raw = re.findall(r"#\s*of\s*ions:\s*[0-9]*", data)[0]
            ...
            band_iterator = 0
            total_count = 0
            for _orientations_count in range(norient):
                for _ions_count in range(nion):
