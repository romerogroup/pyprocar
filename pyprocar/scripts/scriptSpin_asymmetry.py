...
def spin_asymmetry(
    ...
    if show or save2d:
        ...
        p.add_mesh(
            surfaces[0].brillouin_zone.pyvista_obj, style="wireframe", line_width=3.5, color="black"
        )
        for isurface in range(nsurface):
            if not only_spin:
                if mode == "plain":
                    ...
                    p.add_mesh(surfaces[isurface].pyvista_obj, cmap=cmap, clim=[vmin, vmax])
                    p.remove_scalar_bar()
                    text = "Projection"
            else:
                text = "Spin Texture"
        ...
        p.add_axes(xlabel="Kx", ylabel="Ky", zlabel="Kz", line_width=6, labels_off=False)
