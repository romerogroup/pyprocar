...
__email__ = "petavazohi@gmail.com"
    ...
    for i_spc in range(1, len(pdos)+1):
        for i_atom in range(1, len(pdos[f"S{i_spc:02d}"])+1):
            for i_orbital in range(n_orbitals):
                for i_spin in range(n_spins):
                    dos_projected[i_atom-1, 0, i_orbital, i_spin,
                                  :] = pdos[f"S{i_spc:02d}"][f"A{i_atom:04d}"][i_orbital + i_spin * n_orbitals]

    return DensityOfStates(energies, dos_total, dos_projected)


class ElkParser:
    def __init__(self, elk_dir: Union[str, Path] =".", kdirect=True):

        # elk specific inp
        self.elk_dir = Path(elk_dir)
        if not self.elk_dir.exists():
            raise FileNotFoundError(f"The directory {elk_dir} does not exist.")
        self.fin = self.elk_dir / "elk.in"
        if not self.fin.exists():
            raise FileNotFoundError(f"The elk.in file was not found in the directory {elk_dir}")
        self.file_names = []
        self.elkin = None
        self.nbands = None
        ...
        if 21 in self.tasks or 22 in self.tasks:
            ...
            for spc in self.composition:
                for iatom in range(self.composition[spc]):
                    self.file_names.append(self.elk_dir/
    ...
    def _read_bands(self):
        ...
        if np.any([x in self.tasks for x in [20, 21, 22]]):
            ...
            for ifile in range(self.natom):
                with open(self.file_names[ifile], "r") as rf:
                    lines = rf.readlines()
                iline = 0
