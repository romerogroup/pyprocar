...
user_logger = logging.getLogger("user")
            ...
            # self._data[block.name] = block.data
            
        # for block in qe_card_blocks:
        #     if isinstance(block, KPointsCard):
        #         try:
        #             self._kpoints_info = block.parse()
        #         except Exception:
        #             self._kpoints_info = None
        
        
    # @classmethod
    # def register_card(cls, name):
    #     """Decorator to register a parsing function for a card."""
    #     def decorator(func):
    #         cls.card_parsers[name.upper()] = func
    #         return func
    #     return decorator
        
        
    # def parse_qe_cards(self,lines: list[str]) -> dict[str, QEValue]:
    #     cards = {}
    #     i = 0
    #     while i < len(lines):
    #         line = lines[i].strip()
    ...
    #         # Detect a card header (not starting with "&")
    #         if not line.startswith("&"):
    #             header_parts = line.split(maxsplit=1)
    #             card_name = header_parts[0].upper()
    #             options = header_parts[1] if len(header_parts) > 1 else ""

    #             # Gather all lines until blank line or next section/card
    #             block_lines = []
    #             i += 1
    #             while i < len(lines):
    ...
    #                     i += 1
    #                     continue
    #                 # Stop if new card or namelist
    #                 if next_line.startswith("&") or next_line.split()[0].isupper():
    #                     break
    #                 block_lines.append(next_line)
    #                 i += 1

    #             parser = self.card_parsers.get(card_name)
    #             if parser:
    #                 cards[card_name.lower()] = parser(options, block_lines)
    #             else:
    #                 cards[card_name.lower()] = {"options": options, "lines": block_lines}
    #             continue

    #         i += 1  # skip namelists here
    #     return cards
    
    # def parse_atomic_species(self, lines: list[str]) -> Dict[str, QEValue]:
    #     species = {}
    #     for line in lines:
    #         parts = line.split()
    #         label = parts[0]
    ...
    #         species[label] = (mass, pseudopotential)
    #     return {"options": options, "species": species}
    ...
    # # K_POINTS convenience properties
    # @property
    # def kpoints_mode(self) -> Optional[str]:
    #     return self._kpoints_info.mode if self._kpoints_info else None

    # @property
    # def kpoints_nks(self) -> Optional[int]:
    #     return self._kpoints_info.nks if self._kpoints_info else None

    # @property
    # def kpoints(self) -> Optional[np.ndarray]:
    #     return self._kpoints_info.kpoints if self._kpoints_info else None

    # @property
    # def kpoints_weights(self) -> Optional[np.ndarray]:
    #     return self._kpoints_info.weights if self._kpoints_info else None

    # @property
    # def kgrid(self) -> Optional[Tuple[int, int, int]]:
    #     if self._kpoints_info and self._kpoints_info.mode == "automatic":
    #         if (
    #             self._kpoints_info.nk1 is not None
    #             and self._kpoints_info.nk2 is not None
    #             and self._kpoints_info.nk3 is not None
    #         ):
    #             return (
    #                 int(self._kpoints_info.nk1),
    #                 int(self._kpoints_info.nk2),
    #                 int(self._kpoints_info.nk3),
    #             )
    #     return None

    # @property
    # def kshift(self) -> Optional[Tuple[int, int, int]]:
    #     if self._kpoints_info and self._kpoints_info.mode == "automatic":
    #         if (
    #             self._kpoints_info.sk1 is not None
    #             and self._kpoints_info.sk2 is not None
    #             and self._kpoints_info.sk3 is not None
    #         ):
    #             return (
    #                 int(self._kpoints_info.sk1),
    #                 int(self._kpoints_info.sk2),
    #                 int(self._kpoints_info.sk3),
    #             )
    #     return None

    # @property
    # def kpoints_is_gamma(self) -> bool:
    #     return bool(self._kpoints_info.is_gamma) if self._kpoints_info else False
        ...
        #     scf_in_fp,
        # ) = self._resolve_filepaths(
        #     scf_in_filepath=scf_in_filepath,
        #     bands_in_filepath=bands_in_filepath,
        #     pdos_in_filepath=pdos_in_filepath,
        #     kpdos_in_filepath=kpdos_in_filepath,
        #     atomic_proj_xml_filepath=atomic_proj_xml_filepath,
        # )

        # # Per-file wrappers
        # self.scf_in = QEScfIn.from_file(scf_in_fp if scf_in_fp else (self.dirpath / scf_in_filepath))
        # self.scf_out = QEScfOut.from_file(self.scf_out_filepath if self.scf_out_filepath else (self.dirpath / "scf.out"))
        # self.data_xml = QEDataXML.from_file(xml_root_path if xml_root_path else (self.dirpath / "data-file-schema.xml"))
        # self.atomic_proj_xml = QEAtomicProjXML.from_file(
        #     atomic_proj_xml_fp if atomic_proj_xml_fp else (self.dirpath / atomic_proj_xml_filepath)
        # )
        # self.projwfc_out = QEProjwfcOut.from_file(projwfc_out_fp)

        # # efermi
        # self.efermi = self.scf_out.efermi

        # main_xml_root = self.data_xml.root
        # if main_xml_root is not None:
        #     try:
        #         self._parse_magnetization(main_xml_root=main_xml_root)
        #     except Exception:
        #         self.is_non_colinear = False
        #         self.is_spin_calc = False
        #         self.is_spin_orbit_calc = False
        #         self.n_spin = None
        #         self.orbitals = []
        #         self.n_orbitals = 0
        #         self.orbital_names = []
        ...
        #     try:
        #         self._parse_structure(main_xml_root=main_xml_root)
        #     except Exception:
        ...
        #     try:
        #         self._parse_band_structure_tag(main_xml_root=main_xml_root)
        #     except Exception:
        #         self.nkx = None
        #         self.nky = None
        #         self.nkz = None
        ...
        #     try:
        #         self._parse_symmetries(main_xml_root=main_xml_root)
        #     except Exception:
        #         self.rotations = None

        # # Projections
        # if (
        #     getattr(self.atomic_proj_xml, "root", None) is not None
        #     and getattr(self.projwfc_out, "filepath", None) is not None
        # ):
        #     try:
        #         self._parse_wfc_mapping(projwfc_out_filepath=self.projwfc_out.filepath)
        #         self._parse_atomic_projections(
        #             atomic_proj_xml_filepath=self.atomic_proj_xml.filepath
        #         )
        #     except Exception:
        #         self.spd = None
        #         self.spd_phase = None
        ...
        # # DOS
        # if pdos_in_fp and Path(pdos_in_fp).exists():
        #     try:
        #         self.dos = self._parse_pdos(
        #             pdos_in_filepath=pdos_in_fp, dirpath=self.dirpath
        #         )
        #     except Exception:
        #         self.dos = None
        # else:
        ...
        # # Kpath from bands.in if bands calc
        # try:
        #     self.isBandsCalc = (
        #         main_xml_root is not None
        #         and len(main_xml_root.findall(".//input/control_variables/calculation")) > 0
        #         and main_xml_root.findall(".//input/control_variables/calculation")[0].text
        #         == "bands"
        #     )
        # except Exception:
        #     self.isBandsCalc = False

        # self.kticks = None
        # self.knames = None
        # self.kpath = None
        # if self.isBandsCalc and bands_in_fp and Path(bands_in_fp).exists():
        #     try:
        #         with open(bands_in_fp, "r") as f:
        #             self.bandsIn = f.read()
        ...
        # # Build EBS if essential pieces exist
        # if (
        #     self.kpoints is not None
        #     and self.bands is not None
        #     and self.efermi is not None
        #     and self.reciprocal_lattice is not None
        # ):
        ...
        #         labels=self.orbital_names[:-1] if self.orbital_names else None,
    ...
    #     kpdos_in_filepath: Union[str, Path],
    #     atomic_proj_xml_filepath: Union[str, Path],
    # ):
    #     """Resolve common QE filepaths robustly.

    #     Returns tuple: (prefix, xml_root_path, atomic_proj_xml_path, pdos_in_path,
    #     bands_in_path, pdos_out_path, projwfc_out_path, scf_out_path, scf_in_path)
    #     """
    #     scf_in_fp = Path(self.dirpath) / Path(scf_in_filepath).name
    #     scf_out_fp = None
    #     if scf_in_fp.exists():
    #         scf_out_fp = Path(self.dirpath) / f"{scf_in_fp.stem}.out"
    #     else:
    #         candidate = Path(self.dirpath) / "scf.out"
    #         scf_out_fp = candidate if candidate.exists() else None

    #     scf_in_obj = QEScfIn(scf_in_fp)
    #     prefix = scf_in_obj.prefix
    #     outdir = scf_in_obj.outdir

    #     xml_path_candidates = []
    #     if prefix and outdir:
    #         xml_path_candidates.append(Path(self.dirpath) / outdir / f"{prefix}.xml")
    #         xml_path_candidates.append(Path(self.dirpath) / outdir / "data-file-schema.xml")
    #         xml_path_candidates.append(Path(self.dirpath) / f"{prefix}.xml")
    #     else:
    #         xml_path_candidates.append(Path(self.dirpath) / "data-file-schema.xml")
    #         for fp in self.dirpath.glob("*.xml"):
    #             xml_path_candidates.append(fp)

    #     xml_root_path = None
    #     for cand in xml_path_candidates:
    #         if cand.exists():
    #             xml_root_path = cand
    #             break

    #     atomic_proj_xml_path = None
    #     if prefix and outdir:
    #         apath = Path(self.dirpath) / outdir / f"{prefix}.save" / "atomic_proj.xml"
    #         if apath.exists():
    #             atomic_proj_xml_path = apath
    #     if atomic_proj_xml_path is None:
    #         apath2 = Path(self.dirpath) / Path(atomic_proj_xml_filepath).name
    #         atomic_proj_xml_path = apath2 if apath2.exists() else None

    #     projwfc_out_path = None
    #     kpdos_out_path = Path(self.dirpath) / "kpdos.out"
    #     pdos_out_path = Path(self.dirpath) / "pdos.out"
    #     if kpdos_out_path.exists():
    #         projwfc_out_path = kpdos_out_path
    #     elif pdos_out_path.exists():
    #         projwfc_out_path = pdos_out_path

    #     pdos_in_path = Path(self.dirpath) / Path(pdos_in_filepath).name
    #     if not pdos_in_path.exists():
    #         pdos_in_path = None

    #     bands_in_path = Path(self.dirpath) / Path(bands_in_filepath).name
    #     if not bands_in_path.exists():
    #         bands_in_path = None

    #     if xml_root_path and xml_root_path.exists():
    #         try:
    #             root = ET.parse(xml_root_path).getroot()
    #             pref_elems = root.findall(".//input/control_variables/prefix")
    #             if pref_elems:
    #                 prefix = pref_elems[0].text
    #         except Exception:
    #             pass
    ...
    #         prefix,
    #         xml_root_path,
    #         atomic_proj_xml_path,
    #         pdos_in_path,
    #         bands_in_path,
    #         pdos_out_path if pdos_out_path.exists() else None,
    #         projwfc_out_path,
    #         scf_out_fp,
    #         scf_in_fp if scf_in_fp.exists() else None,
    #     )
    ...
    #     # atomic species
    #     atomic_species = main_xml_root.findall(".//output/atomic_species")
    #     if atomic_species:
    #         self.nspecies = len(atomic_species[0])
    #         self.composition = {
    #             species.attrib.get("name", ""): 0 for species in atomic_species[0]
    #         }
    #     else:
    #         self.nspecies = 0
    #         self.composition = {}
    #     self.species_list = list(self.composition.keys())
    #     atomic_structure = main_xml_root.findall(".//output/atomic_structure")
    #     if atomic_structure:
    #         self.ionsCount = int(atomic_structure[0].attrib.get("nat", 0))
    #         self.alat = float(atomic_structure[0].attrib.get("alat", 1.0))
    #     else:
    #         self.ionsCount = 0
    #         self.alat = 1.0
    ...
    #     self.ions = []
    #     atomic_positions = main_xml_root.findall(
    #         ".//output/atomic_structure/atomic_positions"
    #     )
    #     if atomic_positions:
    #         for ion in atomic_positions[0]:
    #             name = ion.attrib.get("name", "")
    #             self.ions.append(name[:2])
    #             if name in self.composition:
    #                 self.composition[name] += 1

    #     self.n_atoms = len(self.ions)

    #     if atomic_positions:
    #         self.atomic_positions = np.array(
    #             [ion.text.split() for ion in atomic_positions[0]], dtype=float
    #         )
    #     else:
    #         self.atomic_positions = None
    #     # in a.u
    #     cell_tags = main_xml_root.findall(".//output/atomic_structure/cell")
    #     if cell_tags:
    #         self.direct_lattice = np.array(
    #             [acell.text.split() for acell in cell_tags[0]], dtype=float
    #         )
    #     else:
    #         self.direct_lattice = None

    #     # in a.u
    #     rec_lat_tags = main_xml_root.findall(
    #         ".//output/basis_set/reciprocal_lattice"
    #     )
    #     if rec_lat_tags:
    #         self.reciprocal_lattice = (2 * np.pi / self.alat) * np.array(
    #             [acell.text.split() for acell in rec_lat_tags[0]], dtype=float
    #         )
    #     else:
    #         self.reciprocal_lattice = None

    #     # Convert to angstrom
    #     if self.direct_lattice is not None:
    ...
    #     self.nk2 = None
    #     self.nk3 = None
    #     starting_k_points = main_xml_root.findall(
    #         ".//output/band_structure/starting_k_points"
    #     )
    #     if starting_k_points:
    #         try:
    #             monkhorst_tag = starting_k_points[0][0]
    #             if "monkhorst_pack" in monkhorst_tag.tag:
    #                 self.nkx = float(monkhorst_tag.attrib.get("nk1", 0.0))
    #                 self.nky = float(monkhorst_tag.attrib.get("nk2", 0.0))
    #                 self.nkz = float(monkhorst_tag.attrib.get("nk3", 0.0))

    #                 self.nk1 = float(monkhorst_tag.attrib.get("k1", 0.0))
    #                 self.nk2 = float(monkhorst_tag.attrib.get("k2", 0.0))
    #                 self.nk3 = float(monkhorst_tag.attrib.get("k3", 0.0))
    #         except Exception:
    #             pass

    #     nks_elems = main_xml_root.findall(".//output/band_structure/nks")
    #     self.nks = int(nks_elems[0].text) if nks_elems else None
    ...
    #     nelec_elems = main_xml_root.findall(".//output/band_structure/nelec")
    #     self.nelec = float(nelec_elems[0].text) if nelec_elems else None
    #     if self.n_spin == 2:
    ...
    #                 dtype=float,
    #             )
    #     # Multiply in 2pi/alat and convert back to crystal basis, guard Nones
    #     if (
    #         self.kpoints is not None
    #         and hasattr(self, "alat")
