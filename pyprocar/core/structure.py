...
class Structure:
    def __init__(self, 
        ...
        if self.lattice is not None and self.fractional_coordinates is not None and atoms is not None:
    ...
    @property
    def alpha(self):
        return np.rad2deg(np.arccos(np.dot(self.lattice[1,:], self.lattice[2, :])/(self.b*self.c)))
    
    @property
    def beta(self):
        return np.rad2deg(np.arccos(np.dot(self.lattice[0,:], self.lattice[2, :])/(self.a*self.c)))
    
    @property
    def gamma(self):
        return np.rad2deg(np.arccos(np.dot(self.lattice[0,:], self.lattice[1, :])/(self.a*self.b)))
    ...
    def transform(self, transformation_matrix=np.array([[1,0,0],[0,1,0],[0,0,1]])):
            ...
            for x in range(-1*scale, scale):
                for y in range(-1*scale, scale):
                    for z in range(-1*scale, scale):
                        p = x*self.lattice[0,:] + y*self.lattice[1,:] + z*self.lattice[2,:] + atom_coord
        ...
        return Structure(atoms=new_atoms,
                         fractional_coordinates=new_fractional,
                         lattice=new_lattice)
