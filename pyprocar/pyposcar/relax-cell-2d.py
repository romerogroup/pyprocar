...
def make_prediction_ab(history, tolerance=0.1, maxStep=0.01):
    ...
    # I will handle the cases of 2, 3 or more data points together
        ...
        if len(history) == 2:
            ...
            parabola = parabola_two_points_derivative(
                param_a[0], energy[0], stress[0], param_a[1], energy[1], stress[1]
            )
        elif len(history) == 3:
            print("There are three data points...")
            parabola = parabola_three_points(
                param_a[0],
                energy[0],
                param_a[1],
                energy[1],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="A lattice optimizer for a 2D"
    ...
    parser.add_argument(
        ...
        "-t",
        type=float,
        help="maximum stress allowed (absolute value, in eV/A)",
        default=0.1,
    )
    parser.add_argument(
        "-s",
        "--script",
        action="store_true",
        help="It creates a sample snippet of a script to run the relaxation. Then quit",
    )
    parser.add_argument(
        "-o", "--discard_old", default=12, help="Discard data older than this value, in iterations"
    )
    parser.add_argument(
        "-m", "--max_iterations", default=100, help="Max number of iterations, before failing"
    )
