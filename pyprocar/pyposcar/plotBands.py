...
class plotBands:
    ...
    def plot(self, orbitals: list[str]) -> None:
        ...
        f.write(
            "pyprocar.bandsplot(PROCAR,outcar = OUTCAR,elimit = elimit,mode = mode,savefig = savefigure,atoms = atoms,orbitals = orbitals, cmap = cmap, vmax = vmax, vmin = vmin)"
        )
        f.write("\n")
        f.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--orbitals", "-o", type=str, default="spd", help="Orbitals asked for ploting Ex = spd"
    )
