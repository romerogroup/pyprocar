...
def test_plot_horizontal_projected_sum_line():
    ...
    total = dos_non_spin_polarized.total
    projected_sum = dos_non_spin_polarized.compute_projected_sum(
        atoms=atoms, orbitals=orbitals, spins=[0]
    )
    plotter = DOSPlotter(orientation="horizontal")
...
def test_plot_horizontal_projected_sum_line_integral_normalized():
    ...
    orbitals = [4, 5, 6, 7, 8]
    projected_sum = dos_non_spin_polarized.compute_projected_sum(
        atoms=atoms, orbitals=orbitals, spins=[0], norm_mode="integral"
    )
    plotter = DOSPlotter(orientation="horizontal")
    plotter.plot(projected_sum)
...
def test_plot_horizontal_total_with_projected_sum_scalars_line():
    ...
    total = dos_non_spin_polarized.total
    projected_sum = dos_non_spin_polarized.compute_projected_sum(
        atoms=atoms, orbitals=orbitals, spins=[0], norm_mode="total_projection"
    )
...
def test_plot_horizontal_total_with_projected_sum_scalars_line_flip_channel_mode_per_channel_colorbar():
    ...
    total = dos_non_spin_polarized.total
    projected_sum = dos_non_spin_polarized.compute_projected_sum(
        atoms=atoms, orbitals=orbitals, spins=[0, 1], norm_mode="total_projection"
    )

    plotter = DOSPlotter(orientation="horizontal")
    plotter.plot(
        total,
        scalars_data=projected_sum,
        scalars_mode="line",
        channel_mode="flip",
...
def test_plot_horizontal_projected_sum_with_grouped_kwargs():
    ...
    projected_sum = dos_non_spin_polarized.compute_projected_sum(
        atoms=atoms, orbitals=orbitals, spins=[0, 1]
    )

    total = dos_non_spin_polarized.total
    plotter = DOSPlotter(orientation="horizontal")
    plot_kwargs = [{"linewidth": 1.0, "alpha": 0.5}, {"linewidth": 2.0, "alpha": 1.0}]
...
def test_non_colinear_plot_total_with_projected_sum_scalars_line():
    ...
    total = dos_non_colinear.total
    projected_sum = dos_non_colinear.compute_projected_sum(
        atoms=atoms, orbitals=orbitals, spins=[0], norm_mode="total_projection"
    )
...
def test_non_colinear_plot_total_with_spin_texture_norm_mode_magnetization_scalars_line():
    ...
    total = dos_non_colinear.total
    magnetization = dos_non_colinear.compute_magnetization(
        atoms=atoms, orbitals=orbitals, norm_mode="magnetization"
    )
...
def test_non_colinear_plot_total_with_mag_norm_mode_magnetization_from_total_line():
    ...
    total = dos_non_colinear.total
    magnetization = dos_non_colinear.compute_magnetization(
        atoms=atoms, orbitals=orbitals, norm_mode="magnetization", from_total=True, fill_value=None
    )
...
def test_non_colinear_plot_total_with_spin_texture_magnitude_scalars_line(**kwargs):
    ...
    total = dos_non_colinear.total
    spin_texture_magnitude = dos_non_colinear.compute_spin_texture_magnitude(
        atoms=atoms, orbitals=orbitals, **kwargs
    )
...
def test_non_colinear_plot_total_with_spin_mag_norm_mode_spin_texture_magnitude_scalars_line(
    **kwargs,
    ...
    total = dos_non_colinear.total
    spin_texture_magnitude = dos_non_colinear.compute_spin_texture_magnitude(
        atoms=atoms, orbitals=orbitals, norm_mode="spin_magnitude", **kwargs
    )
...
def test_non_colinear_plot_total_with_mag_norm_mode_spin_texture_magnitude_scalars_line(**kwargs):
    ...
    total = dos_non_colinear.total
    spin_texture_magnitude = dos_non_colinear.compute_spin_texture_magnitude(
        atoms=atoms, orbitals=orbitals, norm_mode="magnetization", **kwargs
    )
...
def test_non_colinear_plot_total_with_spin_mag_norm_mode_spin_texture_magnitude_from_total_scalars_line(
    **kwargs,
    ...
    total = dos_non_colinear.total
    spin_texture_magnitude = dos_non_colinear.compute_spin_texture_magnitude(
        atoms=atoms, orbitals=orbitals, norm_mode="spin_magnitude", from_total=True, **kwargs
    )
