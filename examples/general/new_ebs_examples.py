...
iband = 0
ispin = 0
spin_texture = ebs.get_property(
    "projected_sum_spin_texture", atoms=[1], orbitals=[4, 5, 6, 7, 8]
).value
...
p = EBSPlanePlotter(
    ebs, normal=(0, 0, 1), origin=(0, 0, 0), grid_interpolation=(20, 20), ax=axes[0, 0]
)
p.plot_vectors_quiver(
    ("spin_texture", spin_texture[:, iband, ispin, :]), cmap="coolwarm", clim=(-0.5, 0.5)
)
p.show_colorbar()
...
p = EBSPlanePlotter(
    ebs, normal=(0, 0, 1), origin=(0, 0, 0), grid_interpolation=(20, 20), ax=axes[1, 0]
)
p.plot_vectors_quiver(
    ("spin_texture", spin_texture[:, iband, ispin, :]),
    cmap="coolwarm",
    clim=(-0.5, 0.5),
    plot_scalar=True,
)
...
p = EBSPlanePlotter(
    ebs, normal=(0, 0, 1), origin=(0, 0, 0), grid_interpolation=(20, 20), ax=axes[0, 1]
)
p.plot_scalars(
    ("spin_texture_divergence", spin_texture_divergence[:, iband, ispin]),
    cmap="coolwarm",
    clim=(-0.5, 0.5),
)
p.show_colorbar()
...
spin_texture_divergence_gradient = ebs.get_property(
    "projected_sum_spin_texture"
).divergence_gradient
p = EBSPlanePlotter(
    ebs, normal=(0, 0, 1), origin=(0, 0, 0), grid_interpolation=(20, 20), ax=axes[1, 1]
)
p.plot_vectors_quiver(
    ("spin_texture_divergence_gradient", spin_texture_divergence_gradient[:, iband, ispin, :]),
    cmap="coolwarm",
    clim=(-0.5, 0.5),
    plot_scalar=True,
)
...
p = EBSPlanePlotter(
    ebs, normal=(0, 0, 1), origin=(0, 0, 0), grid_interpolation=(20, 20), ax=axes[0, 2]
)
p.plot_vectors_quiver(
    ("spin_texture_curl", spin_texture_curl[:, iband, ispin, :]),
    cmap="coolwarm",
    clim=(-0.5, 0.5),
    plot_scalar=True,
)
...
p = EBSPlanePlotter(
    ebs, normal=(0, 0, 1), origin=(0, 0, 0), grid_interpolation=(20, 20), ax=axes[1, 2]
)
p.plot_vectors_quiver(
    ("spin_texture_curl_gradient", spin_texture_curl_gradient[:, iband, ispin, :]),
    cmap="coolwarm",
    clim=(-0.5, 0.5),
    plot_scalar=True,
)
...
for fermi_difference in fermi_differences:
    fs = FermiSurface.from_code(code="vasp", dirpath=NON_SPIN_POLARIZED_DIR, fermi=fermi_difference)
    fs.get_property("fermi_velocity")
    fss.append(fs)
