...
@pytest.fixture
def efermi_spin_polarized(tmp_path):
...
class TestElkFermiInit(BaseTest):
    def test_fermi_from_filepath_non_spin(self, efermi_non_spin):
    ...
    def test_fermi_from_filepath_spin_polarized(self, efermi_spin_polarized):
        """Test loading ElkFermi from EFERMI.OUT file."""
        fermi = ElkFermi(efermi_spin_polarized)
        assert fermi is not None
        assert fermi.filepath == efermi_spin_polarized

    def test_fermi_from_str(self):
...
class TestElkFermiValues(BaseTest):
    ...
    def test_fermi_ev_value_non_spin(self):
