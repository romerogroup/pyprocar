...
@pytest.fixture(params=get_output_files(), ids=lambda p: f"{p.parent.parent.name}/{p.parent.name}")
def output_filepath(request):
    return request.param


class TestAbinitOutputIsFileOfType(BaseTest):
    def test_is_file_of_type_returns_true_for_abinit_output(self, output_filepath):
        from pyprocar.io.abinit import AbinitOutput

        assert AbinitOutput.is_file_of_type(output_filepath) is True

    def test_is_file_of_type_returns_false_for_non_abinit_file(self, tmp_path):
        from pyprocar.io.abinit import AbinitOutput
...
class TestAbinitOutputInit(BaseTest):
    def test_init_from_filepath(self, output_filepath):
        from pyprocar.io.abinit import AbinitOutput

        output = AbinitOutput(output_filepath)
        assert output._filepath is not None

    def test_from_str_creates_instance(self, output_filepath):
        from pyprocar.io.abinit import AbinitOutput
...
class TestAbinitOutputFermi(BaseTest):
    def test_fermi_is_float(self, output_filepath):
        from pyprocar.io.abinit import AbinitOutput

        output = AbinitOutput(output_filepath)
        assert isinstance(output.fermi, float)

    def test_fermi_in_reasonable_range(self, output_filepath):
        from pyprocar.io.abinit import AbinitOutput
...
class TestAbinitOutputNspin(BaseTest):
    def test_nspin_for_non_spin_polarized(self):
        from pyprocar.io.abinit import AbinitOutput
    ...
    def test_nspin_for_spin_polarized(self):
        from pyprocar.io.abinit import AbinitOutput
...
class TestAbinitOutputStructure(BaseTest):
    def test_structure_returns_structure_object(self, output_filepath):
    ...
    def test_lattice_shape_is_3x3(self, output_filepath):
        from pyprocar.io.abinit import AbinitOutput

        output = AbinitOutput(output_filepath)
        assert output.lattice.shape == (3, 3)

    def test_reclat_shape_is_3x3(self, output_filepath):
        from pyprocar.io.abinit import AbinitOutput

        output = AbinitOutput(output_filepath)
        assert output.reclat.shape == (3, 3)

    def test_atoms_is_non_empty_list(self, output_filepath):
        from pyprocar.io.abinit import AbinitOutput
