...
def outcar_filepath(request: pytest.FixtureRequest) -> Path:
    """Fixture that provides OUTCAR file paths for testing."""
    param = request.param
    assert isinstance(param, Path)
    return param
...
@pytest.fixture
def outcar_v544(tmp_path: Path) -> Path:
...
@pytest.fixture
def outcar_v544_ibzkpt(tmp_path: Path) -> Path:
    """Create a temporary PROJCAR file for testing."""
...
@pytest.fixture
def outcar_v544_reclat_issue(tmp_path: Path) -> Path:
    """Create a temporary PROJCAR file for testing."""
...
@pytest.fixture
def outcar_v642(tmp_path: Path) -> Path:
    """Create a temporary PROJCAR file for testing."""
...
@pytest.fixture
def outcar_v643(tmp_path: Path) -> Path:
    """Create a temporary PROJCAR file for testing."""
...
class TestOutcar(BaseTest):
    def test_outcar_v544(self, outcar_v544: Path) -> None:
        outcar = vasp.Outcar(outcar_v544)
        assert outcar.version == "5.4.4"
        assert outcar.version_tuple == (5, 4, 4)
        assert outcar.fermi == 5.6251
        ops = outcar.get_symmetry_operations()
        assert len(ops) == 24
        assert ops[0]["irot"] == 1
        assert ops[0]["rotation"].shape == (3, 3)
        gtrans = ops[0]["gtrans"]
        assert gtrans is not None
        assert gtrans.shape == (3,)
        assert gtrans.dtype == float
        assert gtrans.tolist() == [0.0, 0.0, 0.0]

    def test_outcar_v544_ibzkpt(self, outcar_v544_ibzkpt: Path) -> None:
    ...
    def test_outcar_v544_reclat_issue_get_reciprocal_lattice(
        self, outcar_v544_reclat_issue: Path
    ) -> None:
    ...
    def test_outcar_v642(self, outcar_v642: Path) -> None:
    ...
    def test_outcar_v643(self, outcar_v643: Path) -> None:
    ...
    def test_outcar_from_str(self) -> None:
