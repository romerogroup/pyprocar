...
def simple_eigenvectors(rng):
    ...
    real_part = rng.random((n_kpoints, n_bands, n_basis))
    imag_part = rng.random((n_kpoints, n_bands, n_basis))
    eigenvectors = real_part + 1j * imag_part
...
def band_plot_data():
    ...
    kslist = [list(range(n_kpoints))] * n_bands
    ekslist = [np.sin(np.linspace(0, 2 * np.pi, n_kpoints)) * (i + 1) for i in range(n_bands)]
    wkslist = [np.abs(np.cos(np.linspace(0, 2 * np.pi, n_kpoints))) for _ in range(n_bands)]
...
class TestUnfolder:
    ...
    def test_initialization(self, simple_unfolder):
    ...
    def test_translation_maps_created(self, simple_unfolder):
    ...
    def test_translation_maps_shape(self, simple_unfolder):
        ...
        assert simple_unfolder._trans_rs.shape[0] == 8
        assert simple_unfolder._trans_rs.shape[1] == 3  # 3D vectors

        # trans_indices maps basis elements across translations
        # Shape is (n_translations, n_positions)
        assert simple_unfolder._trans_indices.shape[0] == 8
        assert simple_unfolder._trans_indices.shape[1] == n_positions

    def test_translation_vectors_in_unit_cell(self, simple_unfolder):
    ...
    def test_get_weight_returns_real(self, simple_unfolder, simple_eigenvectors, simple_qpoints):
    ...
    def test_get_weight_bounded(self, simple_unfolder, simple_eigenvectors, simple_qpoints):
    ...
    def test_get_weights_shape(self, simple_unfolder, simple_eigenvectors):
    ...
    def test_get_weights_values_bounded(self, simple_unfolder):
    ...
    def test_get_weight_with_G_vector(self, simple_unfolder, simple_eigenvectors, simple_qpoints):
...
class TestProcarUnfolder:
    ...
    @pytest.fixture
    def procar_unfolder(self, unfolding_supercell_dir):
    ...
    def test_initialization(self, procar_unfolder):
    ...
    def test_procar_parsed(self, procar_unfolder):
    ...
    def test_atoms_loaded(self, procar_unfolder):
    ...
    def test_unfold_returns_weights(self, procar_unfolder):
    ...
    def test_unfold_weights_shape(self, procar_unfolder):
    ...
    def test_unfold_weights_reasonable(self, procar_unfolder):
    ...
    def test_prepare_unfold_basis_creates_eigenvectors(self, procar_unfolder):
    ...
    def test_prepare_unfold_basis_creates_basis_labels(self, procar_unfolder):
    ...
    def test_eigenvectors_normalized(self, procar_unfolder):
        """Test eigenvectors are normalized."""
        procar_unfolder._prepare_unfold_basis()

        # Check normalization along last axis
        norms = np.linalg.norm(procar_unfolder.eigenvectors, axis=2)
...
class TestProcarUnfolderPlot:
    ...
    def test_plot_returns_axes(self, procar_unfolder):
    ...
    def test_plot_with_custom_ylim(self, procar_unfolder):
    ...
    def test_plot_with_show_band(self, procar_unfolder):
    ...
    def test_plot_without_shift_efermi(self, procar_unfolder):
    ...
    def test_plot_with_axis_parameter(self, procar_unfolder):
    ...
    def test_plot_with_custom_color(self, procar_unfolder):
    ...
    def test_plot_saves_tab_file(self, procar_unfolder):
        """Test plot can save tab-separated data file."""
        with tempfile.NamedTemporaryFile(suffix=".csv", delete=False) as f:
            temp_path = f.name

        axes = procar_unfolder.plot(efermi=0.0, savetab=temp_path, show_band=False)
...
class TestPlotBandWeight:
    ...
    def test_plot_without_weights(self, band_plot_data):
    ...
    def test_plot_with_weights_alpha_style(self, band_plot_data):
    ...
    def test_plot_with_weights_width_style(self, band_plot_data):
    ...
    def test_plot_with_weights_color_style(self, band_plot_data):
    ...
    def test_plot_with_efermi(self, band_plot_data):
    ...
    def test_plot_with_efermi_shift(self, band_plot_data):
    ...
    def test_plot_with_custom_yrange(self, band_plot_data):
    ...
    def test_plot_with_external_axis(self, band_plot_data):
    ...
    def test_plot_with_custom_width(self, band_plot_data):
    ...
    def test_plot_with_custom_fatness(self, band_plot_data):
    ...
    def test_plot_with_custom_color(self, band_plot_data):
        """Test plot with custom color."""
        kslist, ekslist, wkslist = band_plot_data
    ...
    def test_plot_with_xticks(self, band_plot_data):
    ...
    def test_plot_with_weight_limits(self, band_plot_data):
    ...
    def test_plot_alpha_normalization(self, band_plot_data):
        """Test that alpha values > 1 are normalized."""
        kslist, ekslist, _ = band_plot_data
        # Create weights that will exceed alpha=1 when multiplied by width
        wkslist = [np.ones(50) * 2.0 for _ in range(4)]  # High weights
...
class TestUnfolderEdgeCases:
    ...
    def test_single_kpoint(
        self, simple_cubic_cell, simple_basis, simple_positions, supercell_matrix_2x2x2, rng
    ):
        """Test with single k-point."""
        n_basis = len(simple_basis)
        eigenvectors = rng.random((1, 2, n_basis)) + 1j * rng.random((1, 2, n_basis))
        norm = np.linalg.norm(eigenvectors, axis=2, keepdims=True)
        eigenvectors = eigenvectors / norm
    ...
    def test_single_band(
        ...
        n_basis = len(simple_basis)
        eigenvectors = rng.random((5, 1, n_basis)) + 1j * rng.random((5, 1, n_basis))
        norm = np.linalg.norm(eigenvectors, axis=2, keepdims=True)
        eigenvectors = eigenvectors / norm
    ...
    def test_non_diagonal_supercell_matrix(
        self, simple_cubic_cell, simple_basis, simple_positions, simple_eigenvectors, simple_qpoints
    ):
    ...
    def test_high_symmetry_kpoints(self, simple_unfolder):
...
class TestPlotBandWeightEdgeCases:
    ...
    def test_single_band_plot(self):
        """Test plot with single band."""
        kslist = [list(range(20))]
        ekslist = [np.sin(np.linspace(0, np.pi, 20))]
        wkslist = [np.ones(20) * 0.5]
    ...
    def test_negative_weights(self):
        ...
        kslist = [list(range(20))]
        ekslist = [np.sin(np.linspace(0, np.pi, 20))]
        wkslist = [np.linspace(-0.5, 0.5, 20)]  # Includes negative values
